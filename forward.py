...
constants: if constants[key]: attributes =
attributes.replace(key, constants[key]) expr =
...
if res:
self.facts.add(res)
def query(self, e): facts =
set([f.expression for f in self.facts]) i = 1
print(f'Querying {e}:') for f in facts:
